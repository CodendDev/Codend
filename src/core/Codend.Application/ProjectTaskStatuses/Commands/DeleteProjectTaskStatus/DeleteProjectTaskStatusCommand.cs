...
public class DeleteProjectTaskStatusCommandHandler : ICommandHandler<DeleteProjectTaskStatusCommand>
{
    ...
    public async Task<Result> Handle(DeleteProjectTaskStatusCommand request, CancellationToken cancellationToken)
    {
        ...
        var statusTasks = await _taskRepository.GetTasksByStatusId(request.StatusId, cancellationToken);
        var statusStories = await _storyRepository.GetStoriesByStatusIdAsync(request.StatusId, cancellationToken);
        var statusEpics = await _epicRepository.GetEpicsByStatusId(request.StatusId, cancellationToken);
